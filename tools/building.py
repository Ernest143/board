...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    Env = env
    Board_Root = os.path.abspath(root_directory)

    print("root {}".format(Board_Root))

    # make an absolute root directory
    # BOARD_ROOT = Board_Root
    # Export('BOARD_ROOT')

    # set BOARD_ROOT in ENV
    Env['BOARD_ROOT'] = Board_Root
    os.environ["BOARD_DIR"] = Board_Root
    # set BSP_ROOT in ENV
    ...
    if tgt_name:
        ...
        SetOption('no_exec', 1)
        try:
            bconfig.CROSS_TOOL, bconfig.PLATFORM = tgt_dict[tgt_name]
            # replace the 'BOARD_CC' to 'CROSS_TOOL'
            os.environ['BOARD_CC'] = bconfig.CROSS_TOOL
        except KeyError:
    ...
    if exec_prefix:
        os.environ['BOARD_CC_PREFIX'] = exec_prefix

    # auto change the 'BOARD_EXEC_PATH' when 'bconfig.EXEC_PATH' get failed
    if not utils.CmdExists(os.path.join(bconfig.EXEC_PATH, bconfig.CC)):
        if 'BOARD_EXEC_PATH' in os.environ:
            # del the 'BOARD_EXEC_PATH' and using the 'EXEC_PATH' setting on bconfig.py
            del os.environ['BOARD_EXEC_PATH']

        try:
            # try to detect toolchains in env
            envm = utils.ImportModule('env')
            # from env import GetSDKPath
            exec_path = envm.GetSDKPath(bconfig.CC)
            if 'gcc' in bconfig.CC:
                exec_path = os.path.join(exec_path, 'bin')

            if os.path.exists(exec_path):
                print('set CC to ' + exec_path)
                bconfig.EXEC_PATH = exec_path
                os.environ['BOARD_EXEC_PATH'] = exec_path
        except Exception as e:
    ...
    utils.ReloadModule(bconfig) # update environment variables to bconfig.py

    # some env variables have loaded in Environment() of SConstruct before re-load bconfig.py;
    # after update bconfig.py's variables, those env variables need to synchronize
    if exec_prefix:
        env['CC'] = bconfig.CC
        env['CXX'] = bconfig.CXX
        env['AS'] = bconfig.AS
        env['AR'] = bconfig.AR
        env['LINK'] = bconfig.LINK
    if exec_path:
    ...
    # parse bconfig.h to get used component
    ...
    if bconfig.PLATFORM in ['gcc'] and str(env['LINKFLAGS']).find('nano.specs') != -1:
    ...
    # # include components
    # objs.extend(SConscript(Board_Root + '/components/SConscript',
    #                        variant_dir=kernel_vdir + '/components',
    #                        duplicate=0,
    #                        exports='remove_components'))
    # include testcases
    # if os.path.isfile(os.path.join(Board_Root, 'examples/utest/testcases/SConscript')):
    #     objs.extend(SConscript(Board_Root + '/examples/utest/testcases/SConscript',
...
def PrepareModuleBuilding(env, root_directory, bsp_directory):
    ...
    Env = env
    Board_Root = root_directory

    # parse bsp bconfig.h to get used component
    PreProcessor = PatchedPreProcessor()
    f = open(bsp_directory + '/bconfig.h', 'r')
    contents = f.read()
    f.close()
    PreProcessor.process_contents(contents)
    BuildOptions = PreProcessor.cpp_namespace
...
def Preprocessing(input, suffix, output = None, CPPPATH = None):
    if hasattr(bconfig, "CPP") and hasattr(bconfig, "CPPFLAGS"):
        if output == None:
            import re
        ...
        for cpppath_item in cpppath:
            inc += ' -I' + cpppath_item
        CPP = bconfig.EXEC_PATH + '/' + bconfig.CPP
        if not os.path.exists(CPP):
            CPP = bconfig.CPP
        CPP += bconfig.CPPFLAGS
        path = GetCurrentDir() + '/'
        os.system(CPP + inc + ' ' + path + input + ' -o ' + path + output)
    else:
        print('CPP tool or CPPFLAGS is undefined in bconfig!')
...
def GroupLibName(name, env):
    ...
    if bconfig.PLATFORM in ['armcc']:
        return name + '_rvds'
    elif bconfig.PLATFORM in ['gcc']:
        return name + '_gcc'
...
def DoBuilding(target, objects):
    ...
    if lib_name:
        ...
        objects_in_group = []
        # print("len projects {}".format(len(Projects)))
        for group in Projects:
...
def GenTargetProject(program = None):
    ...
    if GetOption('target') == 'ua':
        ...
        PrepareUA(Projects, Board_Root, str(Dir('#')))
...
def EndBuilding(target, program = None):
    ...
    if hasattr(bconfig, 'BSP_LIBRARY_TYPE'):
        Env['bsp_lib_type'] = bconfig.BSP_LIBRARY_TYPE

    if hasattr(bconfig, 'dist_handle'):
        Env['dist_handle'] = bconfig.dist_handle

    Env.AddPostAction(target, bconfig.POST_ACTION)
    ...
    if GetOption('make-dist') and program != None:
        MkDist(program, BSP_ROOT, Board_Root, Env, project_name, project_path)
        need_exit = True
    ...
    if not GetOption('help') and not GetOption('target'):
        if not os.path.exists(bconfig.EXEC_PATH):
            print ("Error: the toolchain path (" + bconfig.EXEC_PATH + ") is not exist, please check 'EXEC_PATH' in path or bconfig.py.")
            need_exit = True
...
def GetVersion():
    ...
    rtdef = os.path.join(Board_Root, 'include', 'rtdef.h')
